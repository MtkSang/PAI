...
- (void)dealloc 
{
   ...
   [toolbar_ release], toolbar_ = nil;
    ...
    [leftBarButtonItm release], leftBarButtonItm = nil;
    [actionButton release], actionButton = nil;
    [titleToolbar release], titleToolbar = nil;
...
- (void)loadView 
{
   ...
   CGRect toolbarFrame = CGRectMake(0, 
    ...
    titleToolbar = [[UIBarButtonItem alloc] initWithCustomView:title];
    ...
    NSLog(@"toolbarFarme %f %f %f %f", toolbarFrame.origin.x, toolbarFrame.origin.y, toolbarFrame.size.width, toolbarFrame.size.height);
    botToolbar = [[UIToolbar alloc] initWithFrame:toolbarFrame];
    [botToolbar setAutoresizingMask:UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleTopMargin | UIViewAutoresizingFlexibleRightMargin];
    [botToolbar setBarStyle:UIBarStyleBlackTranslucent];
    [botToolbar setItems:toolbarItems];
    [botToolbar setBackgroundColor:[UIColor clearColor]];
    [[self view] addSubview:botToolbar];
    //Top toolbar
    NSMutableArray *toolbarButtons = [[NSMutableArray alloc] initWithObjects:leftBarButtonItm, space, titleToolbar, space, actionButton, nil];
    ...
    [toolbar_ setBackgroundColor:[UIColor clearColor]];
    [toolbar_ setItems:toolbarButtons];
...
- (void)setTitleWithCurrentPhotoIndex
{
    ...
    float fractionalPage = (scrollView_.contentOffset.x - (pageWidth / 2.0))/ pageWidth;
    NSInteger page = floor(fractionalPage) + 1;
    if (page < 0 || page > photoCount_) {
        return;
    }
    NSString *formatString = NSLocalizedString(@"%1$i of %2$i", @"Picture X out of Y total.");
    NSString *titleStr = [NSString stringWithFormat:formatString, page + 1, photoCount_, nil];
    title.text = titleStr;
    [title sizeToFit];
...
- (void)viewWillAppear:(BOOL)animated 
{
   ...
   [navbar setTranslucent:YES];
    if (statusBarStyle_ != UIStatusBarStyleBlackTranslucent) {
        BOOL isStatusBarHide = [UIApplication sharedApplication].statusBarHidden;
        [[UIApplication sharedApplication] setStatusBarHidden:!isStatusBarHide];
        [self setWantsFullScreenLayout:YES];
...
- (CGRect)frameForPageAtIndex:(NSUInteger)index 
{
   ...
   pageFrame.origin.x = (bounds.size.width * index) + PADDING;
    NSLog(@"scrollView_ %f %f %f %f", bounds.origin.x, bounds.origin.y, bounds.size.width, bounds.size.height);
   return pageFrame;
}
...
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate
{
    ...
    [self setTitleWithCurrentPhotoIndex];
    [self performSelector:@selector(setTitleWithCurrentPhotoIndex) withObject:nil afterDelay:0.00];
    //[self performSelectorOnMainThread:@selector(setTitleWithCurrentPhotoIndex) withObject:nil waitUntilDone:YES];
}
...
- (void)actionSheet:(UIActionSheet *)uias clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    if([[uias buttonTitleAtIndex:buttonIndex] compare:@"Save Photo"] == NSOrderedSame)
    {
        ...
        if ([currentPhotoView isKindOfClass:[KTPhotoView class]]) {
            UIImage *image = [(KTPhotoView*)currentPhotoView getImage];
            if (image) {
                //Save
