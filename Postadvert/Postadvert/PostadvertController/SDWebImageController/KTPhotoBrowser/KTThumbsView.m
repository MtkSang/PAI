...
- (void)layoutSubviews 
{
    ...
    int visibleHeight = visibleBounds.size.height;
     NSLog(@"visibleBounds.x %f", visibleBounds.origin.x);
    ...
    float oneThumbWidth = thumbSize_.width + spaceWidth;
    float contentWidth = thumbCount * oneThumbWidth;
    CGSize contentSize = CGSizeMake(contentWidth, visibleHeight);
    [self setContentSize:contentSize];
    
    NSInteger colsPerView = 1;
    NSInteger topRow = MAX(0, floorf(visibleBounds.origin.x / oneThumbWidth));
    NSInteger bottomRow = topRow + colsPerView;
    
    NSLog(@"rowsPerView %d, TopRow %d. bottomRow %d", colsPerView, topRow, bottomRow);
    CGRect extendedVisibleBounds = CGRectMake(MAX(0,visibleBounds.origin.x),  visibleBounds.origin.y, visibleBounds.size.width + oneThumbWidth, visibleBounds.size.height);
    ...
    NSInteger startAtIndex = MAX(0, topRow );
    NSInteger stopAtIndex = MIN(thumbCount, (bottomRow + itemsPerRow));
    
    // Set our initial origin.
    int x = topRow * oneThumbWidth;
    int y = CELL_MARGIN_BETWEEN_IMAGE ;
    ...
    for (int index = startAtIndex; index < stopAtIndex; index++) {
     ...
     NSLog(@"Start %d Stop %d", startAtIndex, stopAtIndex);
